...
class CI_Input {
	...
	*/
	function _html_entity_decode($str, $charset='UTF-8')
	{
		...
		// Numeric Entities
		$str = preg_replace_callback('~&#x(0*[0-9a-f]{2,5});{0,1}~i', function($m) { return chr(hexdec($m[1])); }, $str);
		$str = preg_replace_callback('~&#([0-9]{2,4});{0,1}~', function($m) { return chr($m[1]); }, $str);
