...
*/
	...
	class Account extends Controller {
		...
		function index($action = 0) {
			if($action == 1) success("Seu novo personagem foi criado!");
			if($action == 2) success("Seu apelido foi definido! Obrigado!");
			if($action == 3) success("Você ultrapassou a quantidade máxima de caracteres por conta.");
			...
			try { $account->find($_SESSION['name']); } catch(Exception $e) {show_error('Houve um problema durante a conta de carregamento. Err code: 220212072010 Futher details: '.$e->getMessage());}
		...
		*/
		function _account_exists($name) {
			...
			try { $account->find($name); } catch(Exception $e) {show_error('Houve um problema durante a conta de carregamento. Err code: 220512072010 Futher details: '.$e->getMessage());}
			if($account->isLoaded()) { $this->form_validation->set_message('_account_exists', 'Já existe uma conta com esse nome.');return false;} else return true;
		}
		...
		function _checkCaptcha($word) {
			if(strtolower($word) == strtolower($_SESSION['captcha']) && !empty($_SESSION['captcha'])) {
				...
				$this->form_validation->set_message('_checkCaptcha', 'A palavra captcha está incorreta.');
				return false;
			}
		}
		...
		function _nicknameExists($name) {
			...
			if($this->account_model->nicknameExists($name)) {
				$this->form_validation->set_message('_nicknameExists', 'Este apelido já existe!.');
				return false;
			}
			else
				return true;
		...
		function _emailExists($email) {
			...
			if($this->account_model->emailExists($email)) {
				$this->form_validation->set_message('_emailExists', 'Este e-mail já é usado por outra conta!');
				return false;
			}
			else
				return true;
		...
		function _characterExists($name) {
			...
			if($this->character_model->characterExists($name)) {
				$this->form_validation->set_message('_characterExists', 'Este nome de personagem já existe, por favor escolha outro!');
				return false;
			}
			else
				return true;
		...
		function _checkDelay() {
			...
			if($config['accountDelay']) {
				if(@(time()-$_SESSION['accountDelay']) > 240) {
					...
					$this->form_validation->set_message('_checkDelay', 'Você não pode criar outra conta após a outra. Por favor, aguarde alguns minutos.');
					return false;
				}
			}
			else
	...
	function strFirst($name) {
		...
		function login($action = 0) {
			...
			if((int) $action == 2) success("Sua conta foi recuperada. Você pode entrar agora.");
			$ide = new IDE;
			$this->load->helper("form");
			$this->load->library("form_validation");
			if($_POST) {
		...
		function _checkCurrentPassword($pass) {
			...
			if($this->account_model->checkPassword($pass)) 
				...
				$this->form_validation->set_message("_checkCurrentPassword", "A senha atual está incorreta.");
				return false;
			}
		}
		...
		function _validKey($key) {
			...
			if($this->account_model->checkKey($_POST['key'], $_POST['email'])) {
				...
				$this->form_validation->set_message("_validKey", "Não foi possível alterar a senha. Certifique-se de que o email e a chave de recuperação são válidos.");
				return false;
			}
		}
