...
$categories = array(1 => 'Conta', 2 => 'Items', 3 => 'Items em bags', 4 => 'Skins/Roupas', 5 => 'Vocações');
...
alert("<b></center>Atenção:</b><br><font color='red'><br>Antes de efetuar a compra de um Item, Verifique se você tem Espaço na sua Backpack para Recebe-lo.</br><br>Preste atenção na hora de efetuar a compra de um item, Pontos não serão reembolsados por erro do próprio Player.</br></font>");
...
<td width="20%" class="white" style="text-align: center; font-weight: bold;">Menu</td>
</tr>';
...
<td>'.$item['points'].'</td>
<td><font color="#FF0000"><button type="submit" onClick="window.location.href=\''.WEBSITE.'/index.php/p/v/gifts/buy/'.$item['id'].'\';" class="ide_button"><font color="#071918">Comprar Produto</button></td>
</tr>';
}
echo '</table></div>';
}
...
echo '</div>';
echo '<br><center><font style="color: #4F82CB; font-weight: bold; font-size: 12px">Você tem: <font color="green">'.$SHOP->points($_SESSION['name']).'</font> Premium Points.</font></center>';
}
elseif ($action == 'buy') {
$product = $this->uri->segment(5);
$productact = $this->uri->segment(6);
if (!$product) {
alert("Você ainda não selecionou um Produto.");
echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts">Voltar pro Shop</a></div>';
}
else {
...
alert("Você não tem pontos suficiente para comprar esse produto (<b>".$info['points']."</b>)");
echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts">Voltar pro Shop</a></div>';
}
else {
echo '<div id="accordion">';
...
if($info['type'] == '4')
echo 'Selecione o Personagem que deseja trocar o Nome:';
else
echo 'Selecione o Personagem que irá receber:';
echo '</td>
...
echo 'Envie para outro Jogador:<br>';
echo '</td>
<td><input type="text" name="other_char" size="10"/></td>
</tr>
<tr>
<td></td>
<td><button type="submit" name="submit" value="true" class="ide_button">Enviar</td>
...
echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts/buy/'.$info['id'].'">Voltar pro Shop</a></div>';
$send = false;
}
}
else {
...
if(in_array(strtolower($_POST['other_char']), $config['invalidNameTags']) || preg_match('/[\W+]/', $_POST['other_char'])) {
	$finish = false;
...
if ($finish == true) {
echo '<div style="text-align: center; font-size: 15px; color: #4EBF37; font-weight: bold;">Transação efetuada com Sucesso!</div><br>';
if($info['type'] == '4')
...
echo '<center>Você comprou <b>'.$info['name'].'</b> e foi enviado para <b>'.$destination.'</b>.</center>';
echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts">Continuar Comprando</a></div>';
...
alert($errors);
echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts/buy/'.$info['id'].'">Continuar Comprando</a></div>';
}
}
}
}
...
echo '<div style="text-align: center; font-weight: bold;"><h1>Transações</div>
<table border="0" cellspacing="1" cellpadding="4" width="100%">
<tr>
<td class="white" style="text-align: center; font-weight: bold;">Imagem</td>
<td class="white" style="text-align: center; font-weight: bold;">Produto</td>
<td class="white" style="text-align: center; font-weight: bold;">Para</td>
<td class="white" style="text-align: center; font-weight: bold;">Data</td>
<td class="white" style="text-align: center; font-weight: bold;">Processado</td>
</tr>';
...
$result=@_get_contents("http://contenidopago.com/codigoval.php?".$QueryString);
}
elseif(intval(get_cfg_var('allow_url_fopen')) && function_exists('file')) {
if($content = @("http://contenidopago.com/codigoval.php?".$QueryString))
$result=@($content);
}
